        ...
        prototype : {
            ...
            _resolveNameTokens : function(glob) {
                return glob.replace(Paths.DEFAULTS.nameTokenPattern, (match) => {
                    ...
                    if ( ! this.hasPath(name)) {
                        throw new Error("Path name :\"" + name + "\" cannot be found, given glob: \"" + glob + "\".");
                    }
            ...
            _processGlob : function(glob, options) {
                ...
                if (isArray(glob)) {
                    ...
                    glob.forEach((globEntry) => {
                        filteredGlob.push(this._filterGlob(globEntry, options));
                    });
            ...
            getPath : function(name) {
                ...
                if ( ! this.hasPath(name) ) {
                    throw new Error("The given name cannot be found: \"" + name +"\"");
                }
            ...
            setPath : function(name, glob, options) {
                ...
                if ( ! isString(name) || isEmptyString(name) ) {
                    throw new Error("The name must be a non-empty string, got:\"" + name + "\".");
                }

                if ( ! isString(glob) && ! isArray(glob) ) {
                    throw new Error("The glob most be a string or an array, got: \"" + typeof glob + "\".");
                }
